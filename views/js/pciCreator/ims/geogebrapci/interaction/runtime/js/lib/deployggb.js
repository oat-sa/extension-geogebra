/*
  ...
  This version was modified by Janfix : search - modified by Janfix - to see the modifications
*/
(function() {
    if (typeof window.GGBApplet == "function") { console.warn("deployggb.js was loaded twice"); return }
    ...
    var GGBApplet = function() {
            ...
            if (p !== null) {
                switch (typeof p) {
                    case "number":
                        ...
                        break;
                    case "string":
                        if (p.match(new RegExp("^[0-9]\\.[0-9]+$"))) { ggbVersion = p } else { appletID = p }
                        break;
                    case "object":
                        if (typeof p.is3D !== "undefined") { views = p } else { parameters = p }
                        break;
                    case "boolean":
                        html5NoWebSimple = p;
        ...
        if (views === null) { views = { is3D: false, AV: false, SV: false, CV: false, EV2: false, CP: false, PC: false, DA: false, FI: false, PV: false, macro: false }; if (parameters.material_id !== undefined && !html5NoWebSimpleParamExists) { html5NoWebSimple = true } }
        if (appletID !== null && parameters.id === undefined) { parameters.id = appletID }
        var jnlpFilePath = "";
        var html5Codebase = "";
        ...
        if (parameters.height !== undefined) { parameters.height = Math.round(parameters.height) }
        if (parameters.width !== undefined) { parameters.width = Math.round(parameters.width) }
        var parseVersion = function(d) { return parseFloat(d) > 4 ? parseFloat(d) : 5 };
        ...
        applet.setHTML5CodebaseVersion = function(version, offline) {
            ...
            if (numVersion !== NaN && numVersion < 5) { console.log("The GeoGebra HTML5 codebase version " + numVersion + " is deprecated. Using version latest instead."); return }
            html5OverwrittenCodebaseVersion = version;
            setDefaultHTML5CodebaseForVersion(version, offline)
        };
        applet.getHTML5CodebaseVersion = function() { return html5CodebaseVersion };
        applet.getParameters = function() { return parameters };
        applet.setFontsCSSURL = function(url) { fonts_css_url = url };
        ...
        applet.inject = function() {
            ...
            function isOwnIFrame() { return window.frameElement && window.frameElement.getAttribute("data-singleton") }
            var type = "auto";
            var container_ID = parameters.id;
            var container;
            var noPreview = false;
            for (var i = 0; i < arguments.length; i++) { var p = arguments[i]; if (typeof p === "string") { p = p.toLowerCase(); if (p.match(/^(prefer)?(java|html5|compiled|auto|screenshot)$/)) { type = p } else { container_ID = arguments[i] } } else if (typeof p === "boolean") { noPreview = p } else if (p instanceof HTMLElement) { container = p } }
            continueInject();

            function continueInject() {
                if (!initComplete) { setTimeout(continueInject, 200); return }
                type = detectAppletType(type);
                var appletElem = document.getElementsByClassName(container_ID)[0]; //modified by Janfix
                // var appletElem = container || document.getElementById(container_ID); //original
                console.log(appletElem);
                console.log(parameters);
                if (!appletElem) { console.log("possibly bug on ajax loading? "); return }
                ...
                if (parameters.height === undefined && appletElem.clientHeight) {
                    ...
                    console.log("Tracker02")
                }
                if (!(parameters.width && parameters.height) && type === "html5") {
                    delete parameters.width;
                    delete parameters.height
                    console.log("Tracker03")
                }
                loadedAppletType = type;
                //console.log(parameters) // Width and Height are undefined because of Tracker03!!!

                if (type === "screenshot") { injectScreenshot(appletElem, parameters) } else {
                    var playButton = false;
                    if (parameters.hasOwnProperty("playButton") && parameters.playButton || parameters.hasOwnProperty("clickToLoad") && parameters.clickToLoad) { playButton = true } else if (parameters.hasOwnProperty("playButtonAutoDecide") && parameters.playButtonAutoDecide) { playButton = (!isInIframe() || isOwnIFrame()) && isMobileDevice() }
                    if (playButton) {
                        loadedAppletType = "screenshot";
                        injectPlayButton(appletElem, parameters, noPreview, type)
                    } else { injectHTML5Applet(appletElem, parameters, noPreview) }
                }
            }
            return
        };
        ...
        function isMobileDevice() { if (parameters.hasOwnProperty("screenshotGenerator") && parameters.screenshotGenerator) { return false } return Math.max(screen.width, screen.height) < 800 }
        applet.getViews = function() { return views };
        applet.isJavaInstalled = function() { return false };
        var getDefaultApiUrl = function() { var host = location.host; if (host.match(/alpha.geogebra.org/) || host.match(/groot.geogebra.org/)) { return "https://groot.geogebra.org:5000" } if (host.match(/beta.geogebra.org/)) { return "https://api-beta.geogebra.org" } if (host.match(/stage.geogebra.org/)) { return "https://api-stage.geogebra.org" } return "https://api.geogebra.org" };
        var fetchParametersFromApi = function(successCallback, materialsApiUrl) {
            ...
            var onSuccess = function(text) {
                ...
                if (!item || !item.url) { onError(); return }
        ...
        function updateAppletSettings(settings) {
            ...
            ["enableLabelDrags", "enableShiftDragZoom", "enableRightClick"].forEach((function(name) { settings[name] = !!settings[name] }));
            parameterNames.forEach((function(name) { if (parameters[name] === undefined && settings[name] !== undefined) { parameters[name] = settings[name] } }));
            if (parameters.showToolBarHelp === undefined) { parameters.showToolBarHelp = parameters.showToolBar }
        }

        function sendCorsRequest(url, onSuccess, onError) {
            var xhr = new XMLHttpRequest;
            xhr.open("GET", url);
            xhr.onload = function() { onSuccess(xhr.responseText) };
            xhr.onerror = onError;
            xhr.send()
        }
        applet.isHTML5Installed = function() { return true };
        applet.getLoadedAppletType = function() { return loadedAppletType };
        ...
        applet.removeExistingApplet = function(appletParent, showScreenshot) {
            var i;
            if (typeof appletParent === "string") { appletParent = document.getElementById(appletParent) }
            ...
            for (i = 0; i < appletParent.childNodes.length; i++) {
                ...
                if (className === "applet_screenshot") {
                    if (showScreenshot) {
                        ...
                        loadedAppletType = "screenshot"
                    } else { currentChild.style.display = "none" }
        ...
        applet.refreshHitPoints = function() { if (parseVersion(ggbHTML5LoadedCodebaseVersion) >= 5) { return true } var app = applet.getAppletObject(); if (app) { if (typeof app.recalculateEnvironments === "function") { app.recalculateEnvironments(); return true } } return false };
        applet.startAnimation = function() { var app = applet.getAppletObject(); if (app) { if (typeof app.startAnimation === "function") { app.startAnimation(); return true } } return false };
        applet.stopAnimation = function() { var app = applet.getAppletObject(); if (app) { if (typeof app.stopAnimation === "function") { app.stopAnimation(); return true } } return false };
        applet.getAppletObject = function() { var appName = parameters.id !== undefined ? parameters.id : "ggbApplet"; return window[appName] };
        ...
        var valBoolean = function(value) { return value && value !== "false" };
        var injectHTML5Applet = function(appletElem, parameters, noPreview) {
            if (parseVersion(html5CodebaseVersion) <= 4.2) { noPreview = true }
            ...
            if (parameters.width !== undefined) {
                ...
                if (parseVersion(html5CodebaseVersion) <= 4.4) {
                    if (valBoolean(parameters.showToolBar)) { parameters.height -= 7 }
                    if (valBoolean(parameters.showAlgebraInput)) { parameters.height -= 37 }
                    ...
                    if (valBoolean(parameters.showToolBar) || valBoolean(parameters.showMenuBar)) {
                        if (parameters.hasOwnProperty("customToolBar")) { parameters.customToolbar = parameters.customToolBar }
            ...
            for (var key in parameters) { if (parameters.hasOwnProperty(key) && key !== "appletOnLoad") { article.setAttribute("data-param-" + key, parameters[key]) } }
            if (fonts_css_url) { article.setAttribute("data-param-fontscssurl", fonts_css_url) }
            applet.resize = function() {
            ...
            if (typeof jQuery === "function") {
                console.log("TRACKER 4bis")
                jQuery(window).resize((function() { applet.resize() }))

            } else {
                var oldOnResize = null;

                if (window.onresize !== undefined && typeof window.onresize === "function") { oldOnResize = window.onresize }
                window.onresize = function() { applet.resize(); if (typeof oldOnResize === "function") { oldOnResize() } }
            }
            ...
            if (!noPreview && parameters.width !== undefined) {
                ...
                } else { var previewContainer = previewPositioner.querySelector(".ggb_preview") }
                if (window.GGBT_spinner) { window.GGBT_spinner.attachSpinner(previewPositioner, "66%") }
                if (parseVersion(html5CodebaseVersion) >= 5) {
                    parameters.appletOnLoad = function(api) {
                        ...
                        if (window.GGBT_spinner) { window.GGBT_spinner.removeSpinner(previewPositioner) }
                        if (window.GGBT_wsf_view) { $(window).trigger("resize") } else { window.onresize() }
                        oriAppletOnload(api)
                    };
                    if (!preRendered) { previewPositioner.appendChild(previewContainer) }
                } else { article.appendChild(previewContainer) }
                previewPositioner.appendChild(article);
                if (!preRendered) { appletElem.appendChild(previewPositioner) }
                setTimeout((function() { applet.resize() }), 1)
            } else {
                ...
                article.style.width = "100%"; //Changed by Janfix article is the last container - redimension top toolbar
                appletScaler.appendChild(article); //Follow article NOW ! 
                appletElem.appendChild(appletScaler);
                //console.log(applet);
                //console.log(article);
                //console.log(appletScaler);
                parameters.appletOnLoad = function(api) {
                    //console.log(api)
            ...
            function renderGGBElementWithParams(article, parameters) {
                ...
                if (parameters && typeof parameters.appletOnLoad === "function" && typeof renderGGBElement === "function") {
                    ...
                    //console.log("JPTRACKER FIND RENDER GGB ELEMENT")
                } else { renderGGBElement(article) }
            ...
            function renderGGBElementOnTube(a, parameters) {
                ...
                if (typeof renderGGBElement === "undefined") {
                    ...
                    if (html5AppletsToProcess === null) { html5AppletsToProcess = [] }
                    ...
                    if (parseVersion(html5CodebaseVersion) < 5) { a.className += " geogebraweb" }
                } else { renderGGBElementWithParams(a, parameters) }
            }
            ...
            if (loadScript) {
                ...
                if (ggbHTML5LoadedScript !== null) { var el = document.querySelector('script[src="' + ggbHTML5LoadedScript + '"]'); if (el !== undefined && el !== null) { el.parentNode.removeChild(el) } }
                var script = document.createElement("script");
                var scriptLoaded = function() { renderGGBElementOnTube(article, parameters) };
                ...
                if (!html5OverwrittenCodebase && (!html5OverwrittenCodebaseVersion || html5OverwrittenCodebaseVersion == "5.0")) {
                    ...
                    if (html5CodebaseIsWebSimple) { webSimple.succeeded = webSimple.succeeded || webSimple() } 
                    else { 
                        console.log(web3d);
        ...
        var injectScreenshot = function(appletElem, parameters, showPlayButton) {
            ...
            if (showPlayButton) { appletElem.appendChild(getPlayButton()); if (!window.GGBT_wsf_view) { appletElem.style.position = "relative" } } else if (window.GGBT_spinner) { window.GGBT_spinner.attachSpinner(previewPositioner, "66%") }
            ...
            applet.resize = function() { resizeScreenshot(appletElem, previewContainer, previewPositioner, showPlayButton) };
            if (typeof jQuery === "function") { jQuery(window).resize((function() { applet.resize() })) } else {
                var oldOnResize = null;
                if (window.onresize !== undefined && typeof window.onresize === "function") { oldOnResize = window.onresize }
                window.onresize = function() { applet.resize(); if (typeof oldOnResize === "function") { oldOnResize() } }
            }
        ...
        function resizeScreenshot(appletElem, previewContainer, previewPositioner, showPlayButton, oldOnResize) {
            if (!appletElem.contains(previewContainer)) { return }
            if (typeof window.GGBT_wsf_view === "object" && window.GGBT_wsf_view.isFullscreen()) {
                if (appletElem.id !== "fullscreencontent") { return }
            ...
            if (typeof window.GGBT_wsf_view === "object" && window.GGBT_wsf_view.isFullscreen()) { GGBAppletUtils.positionCenter(appletElem) }
            if (typeof window.GGBT_ws_header_footer === "object") { window.GGBT_ws_header_footer.setWsScrollerHeight() }
            if (typeof oldOnResize === "function") { oldOnResize() }
        }
        applet.onExitFullscreen = function(fullscreenContainer, appletElem) { appletElem.appendChild(fullscreenContainer) };
        var injectPlayButton = function(appletElem, parameters, noPreview, type) {
            injectScreenshot(appletElem, parameters, true);
            var play = function() {
                var elems = [];
                for (i = 0; i < appletElem.childNodes.length; i++) { elems.push(appletElem.childNodes[i]) }
                if (window.GGBT_wsf_view) {
                    ...
                    if (oldcontent.length > 0) {
                        ...
                        window.onresize()
                    } else { injectHTML5Applet(content, parameters, false) }
                    window.GGBT_wsf_view.launchFullScreen(container)
                } else {
                    loadedAppletType = type;
                    injectHTML5Applet(appletElem, parameters, false)
                }
                if (!window.GGBT_wsf_view) { for (i = 0; i < elems.length; i++) { appletElem.removeChild(elems[i]) } }
            };
            ...
            if (isMobileDevice() && window.GGBT_wsf_view) { $(".wsf-element-fullscreen-button").remove() }
        };
        ...
        var createScreenShotDiv = function(oriWidth, oriHeight, borderColor, showPlayButton) {
            ...
            var bc = "lightgrey";
            if (borderColor !== undefined) { if (borderColor === "none") { bc = "transparent" } else { bc = borderColor } }
            ...
            if (previewImagePath !== null) { preview.setAttribute("src", previewImagePath) }
            ...
            if (previewLoadingPath !== null) {
                ...
                if (!showPlayButton) {
                    ...
                    pWidth = 360;
                    if (pWidth > oriWidth / 4 * 3) { pWidth = oriWidth / 4 * 3 }
                    pHeight = pWidth / 5.8;
        ...
        var detectAppletType = function(preferredType) { preferredType = preferredType.toLowerCase(); if (preferredType === "html5" || preferredType === "screenshot") { return preferredType } return "html5" };
        ...
        var setDefaultHTML5CodebaseForVersion = function(version, offline) {
            ...
            if (offline) { setHTML5CodebaseInternal(html5CodebaseVersion, true); return }
            var hasWebSimple = !html5NoWebSimple;
            if (hasWebSimple) { var v = parseVersion(html5CodebaseVersion); if (!isNaN(v) && v < 4.4) { hasWebSimple = false } }
            var protocol, codebase;
            if (window.location.protocol.substr(0, 4) === "http") { protocol = window.location.protocol } else { protocol = "http:" }
            var index = html5CodebaseVersion.indexOf("//");
            if (index > 0) { codebase = html5CodebaseVersion } else if (index === 0) { codebase = protocol + html5CodebaseVersion } else { codebase = "https://www.geogebra.org/apps/5.0.674.0/" }
            for (var key in modules) { if (html5CodebaseVersion.slice(modules[key].length * -1) === modules[key] || html5CodebaseVersion.slice((modules[key].length + 1) * -1) === modules[key] + "/") { setHTML5CodebaseInternal(codebase, false); return } }
        ...
        var setHTML5CodebaseInternal = function(codebase, offline) {
            if (codebase.requirejs) { html5Codebase = codebase; return }
            if (codebase.slice(-1) !== "/") { codebase += "/" }
            html5Codebase = codebase;
            if (offline === null) { offline = codebase.indexOf("http") === -1 }
            ...
            if (folders.length > 1) {
                ...
                } else if (modules.indexOf(folders[folders.length - 2]) >= 0) { html5CodebaseScript = folders[folders.length - 2] + ".nocache.js" }
            }
            folders = codebase.split("/");
            html5CodebaseVersion = folders[folders.length - 3];
            if (html5CodebaseVersion.substr(0, 4) === "test") { html5CodebaseVersion = html5CodebaseVersion.substr(4, 1) + "." + html5CodebaseVersion.substr(5, 1) } else if (html5CodebaseVersion.substr(0, 3) === "war" || html5CodebaseVersion.substr(0, 4) === "beta") { html5CodebaseVersion = "5.0" }
        ...
        if (parameters.material_id !== undefined) { fetchParametersFromApi(continueInit, parameters.apiUrl) } else { continueInit() }

        function continueInit() {
            var html5Version = ggbVersion;
            if (html5OverwrittenCodebaseVersion !== null) { html5Version = html5OverwrittenCodebaseVersion } else { if (parseFloat(html5Version) < 5) { html5Version = "5.0" } }
            setDefaultHTML5CodebaseForVersion(html5Version, false);
            if (html5OverwrittenCodebase !== null) { setHTML5CodebaseInternal(html5OverwrittenCodebase, isHTML5Offline) }
            initComplete = true
        }
        //Applet basic
        return applet
    ...
    var GGBAppletUtils = function() {
        ...
        function isFlexibleWorksheetEditor() { return window.GGBT_wsf_edit !== undefined }
        ...
        function scaleElement(el, scale) {
            if (scale != 1) {
                ...
                if (el.querySelector(".ggb_preview") !== null) { el.querySelector(".ggb_preview").style.maxWidth = "initial" }
                if (el.querySelectorAll(".ggb_preview img")[0] !== undefined) { el.querySelectorAll(".ggb_preview img")[0].style.maxWidth = "initial" }
                if (el.querySelectorAll(".ggb_preview img")[1] !== undefined) { el.querySelectorAll(".ggb_preview img")[1].style.maxWidth = "initial" }
            } else {
        ...
        function getWidthHeight(appletElem, appletWidth, allowUpscale, autoHeight, noBorder, scaleContainerClass) {
            var container = null;
            console.log("Tracker 15")
            if (scaleContainerClass != undefined && scaleContainerClass != "") { var parent = appletElem.parentNode; while (parent != null) { if ((" " + parent.className + " ").indexOf(" " + scaleContainerClass + " ") > -1) { container = parent; break } else { parent = parent.parentNode } } }
            var myWidth = 0,
                myHeight = 0,
                windowWidth = 0,
                border = 0,
            ...
            if (appletElem) {
                ...
                if ((allowUpscale === undefined || !allowUpscale) && appletWidth > 0 && appletWidth + border < myWidth) { myWidth = appletWidth } else { myWidth -= border }
                if (typeof window.GGBT_wsf_view === "object" && window.GGBT_wsf_view.isFullscreen() && (allowUpscale === undefined || !allowUpscale)) { myHeight -= borderTop }
            }
            return { width: myWidth, height: myHeight }
        }

        function calcScale(parameters, appletElem, allowUpscale, showPlayButton, scaleContainerClass) {
            if (parameters.isScreenshoGenerator) { return 1 }
            ...
            if (appletWidth === undefined) {
                ...
                if (article) {
                    ...
                    console.log("TRACKER 23 VERITABLE MESURE APPLET")
                    console.log(appletWidth)
                }
            }
        ...
        function getScale(parameters, appletElem, showPlayButton) {
            ...
            if (parameters.hasOwnProperty("allowUpscale")) { allowUpscale = parameters.allowUpscale }
            if (parameters.hasOwnProperty("scale")) { scale = parseFloat(parameters.scale); if (isNaN(scale) || scale === null || scale === 0) { scale = 1 } if (scale > 1) { allowUpscale = true } }
            if (appletElem && typeof window.GGBT_wsf_view === "object" && window.GGBT_wsf_view.isFullscreen()) { allowUpscale = true }
            if (!(parameters.hasOwnProperty("disableAutoScale") && parameters.disableAutoScale)) { autoScale = calcScale(parameters, appletElem, allowUpscale, showPlayButton, parameters.scaleContainerClass) } else { return scale }
            if (allowUpscale && (!parameters.hasOwnProperty("scale") || scale === 1)) { return autoScale } else { return Math.min(scale, autoScale) }
        }
        ...
        function positionCenter(appletElem) {
            ...
            if (calcVerticalBorder < 0) { calcVerticalBorder = 0 }
            appletElem.style.position = "relative";
            if (window.GGBT_wsf_view.getCloseBtnPosition() === "closePositionRight") {
                if (calcHorizontalBorder < 40) { appletElem.style.left = "40px" } else { appletElem.style.left = calcHorizontalBorder + "px" }
                appletElem.style.top = calcVerticalBorder + "px"
            } else if (window.GGBT_wsf_view.getCloseBtnPosition() === "closePositionTop") {
                if (calcVerticalBorder < 40) { appletElem.style.top = "40px" } else { appletElem.style.top = calcVerticalBorder + "px" }
        ...
        function responsiveResize(appletElem, parameters) {
            ...
            if (article) {
                if (typeof window.GGBT_wsf_view === "object" && window.GGBT_wsf_view.isFullscreen()) {
                    if (parameters.id !== article.getAttribute("data-param-id")) { return }
                    window.GGBT_wsf_view.setCloseBtnPosition(appletElem)
                }
                if (article.parentElement && /fullscreen/.test(article.parentElement.className)) { return }
                var scale = getScale(parameters, appletElem);
                if (isFlexibleWorksheetEditor()) { article.setAttribute("data-param-scale", scale) }
                var scaleElem = null;
                for (var i = 0; i < appletElem.childNodes.length; i++) { if (appletElem.childNodes[i].className !== undefined && appletElem.childNodes[i].className.match(/^applet_scaler/)) { scaleElem = appletElem.childNodes[i]; break } }
                if (scaleElem !== null && scaleElem.querySelector(".noscale") !== null) { return }
                ...
                if (typeof window.GGBT_wsf_view === "object" && window.GGBT_wsf_view.isFullscreen()) { positionCenter(appletElem) }
                if (window.GGBT_wsf_view && !window.GGBT_wsf_view.isFullscreen()) { window.GGBT_wsf_general.adjustContentToResize($(article).parents(".content-added-content")) }
            }
        }
    ...
    GGBAppletUtils.makeModule = function(name, permutation) {
        ...
        function webModule() {
            ...
            function q() { var a = o.location.search; return a.indexOf(K) != -1 || a.indexOf(L) != -1 }
            ...
            webModule.__gwtStartLoadingFragment = function() { return null };
            webModule.__gwt_isKnownPropertyValue = function() { return false };
            webModule.__gwt_getMetaProperty = function() { return null };
            var s = null;
            ...
            function A(k) {
                function l(a) {
                    function b() { if (typeof p.readyState == Y) { return typeof p.body != Y && p.body != null } return /loaded|complete/.test(p.readyState) }
                    var c = b();
                    if (c) { a(); return }
                    ...
                    function d() {
                        if (!c) {
                            ...
                            c = true;
                            a();
                            if (p.removeEventListener) { p.removeEventListener(Z, d, false) }
                            if (e) { clearInterval(e) }
                        }
                    }
                    if (p.addEventListener) { p.addEventListener(Z, d, false) }
                    var e = setInterval((function() { d() }), $)
                }
                ...
                webModule.onScriptDownloaded = function(a) { l((function() { m(a) })) };
                var n = p.createElement(cb);
                n.src = k;
                if (webModule.__errFn) { n.onerror = function() { webModule.__errFn(M, new Error(gb + code)) } }
                p.getElementsByTagName(hb)[P].appendChild(n)
            }
            webModule.__startLoadingFragment = function(a) { return D(a) };
            ...
            function B() {
                ...
                for (var g = P, h = f.length; g < h; ++g) {
                    ...
                    if (j) {
                        ...
                        } else if (j == pb) { k = i.getAttribute(nb); if (k) { try { d = eval(k) } catch (a) { alert(qb + k + rb) } } } else if (j == sb) { k = i.getAttribute(nb); if (k) { try { e = eval(k) } catch (a) { alert(qb + k + tb) } } }
                    }
                }
            ...
            function C() {
                function e(a) { var b = a.lastIndexOf(ub); if (b == -1) { b = a.length } var c = a.indexOf(vb); if (c == -1) { c = a.length } var d = a.lastIndexOf(wb, Math.min(c, b)); return d >= P ? a.substring(P, d + Q) : W }
                ...
                if (k == W) { k = h() }
                if (k == W) { k = i() }
                if (k == W && j()) { k = e(p.location.href) }
                k = f(k);
                return k
            }

            function D(a) { if (a.match(/^\//)) { return a } if (a.match(/^[a-zA-Z]+:\/\//)) { return a } return webModule.__moduleBase + a }
            ...
            function F() {
                ...
                function h(a, b) {
                    var c = f;
                    for (var d = P, e = a.length - Q; d < e; ++d) { c = c[a[d]] || (c[a[d]] = []) }
                    c[a[e]] = b
                }
                var i = [];
                var j = [];
                ...
                j[Db] = function() { var a = navigator.userAgent.toLowerCase(); var b = p.documentMode; if (function() { return a.indexOf(Eb) != -1 }()) return Fb; if (function() { return a.indexOf(Gb) != -1 && (b >= $ && b < Hb) }()) return Ib; if (function() { return a.indexOf(Gb) != -1 && (b >= Jb && b < Hb) }()) return Kb; if (function() { return a.indexOf(Gb) != -1 && (b >= Lb && b < Hb) }()) return Mb; if (function() { return a.indexOf(Nb) != -1 || b >= Hb }()) return Ob; return Fb };
                i[Db] = { gecko1_8: P, ie10: Q, ie8: Pb, ie9: Qb, safari: Rb };
                __gwt_isKnownPropertyValue = function(a, b) { return b in i[a] };
                webModule.__getPropMap = function() { var a = {}; for (var b in i) { if (i.hasOwnProperty(b)) { a[b] = k(b) } } return a };
            ...
            B();
            webModule.__moduleBase = "https://www.geogebra.org/apps/5.0.674.0/" + name + "/";
    ...
    if (typeof window.web3d !== "function") {
        window.web3d = GGBAppletUtils.makeModule("web3d", "56611B7003D6D8F0935CF29C9C543765");
    }
    if (typeof window.webSimple !== "function") { window.webSimple = GGBAppletUtils.makeModule("webSimple", "39F8760F21A8D74BA19BA6D5E55CDF79") }
    window.GGBApplet = GGBApplet;
    //$(".GGBPCI").GGBApplet = GGBApplet;
